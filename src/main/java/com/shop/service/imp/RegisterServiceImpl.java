...
import com.shop.dao.UserGroupDao;
...
public class RegisterServiceImpl implements RegisterService {
    ...
    public Object register(RegisterInfo registerInfo) {
        ...
        Integer userId=registerInfo.getUserId();
        userGroupDao.addUserToDefaultGroup(userId);
    ...
    public String getPhoneNumberByUserName(String userName){
        ...
        String phoneNumber=userInfoDao.getPhoneNumberByUserName(userName);
        return phoneNumber;
    }

    public boolean resetPassword(ResetInfo resetInfo){
        String validCode=resetInfo.getValidCode();
        String realValidCode=resetInfo.getRealValidCode();
        //检查验证码
        if((realValidCode==null)||!realValidCode.equalsIgnoreCase(validCode)){
            return false;
        }
        String userName=resetInfo.getUserName();
        String password=resetInfo.getPassword();
        String phone=resetInfo.getResetPhone();
        password= MD5tool.createPassword(password);
        return userInfoDao.updatePassword(userName,password,phone)>0;
    }
}
